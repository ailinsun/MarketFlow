...
- [check_public.py](check_public.py) scans identifiers, private references, unresolved placeholders, imports and relative links. Add `--history` after creating the public commit to scan every reachable historical blob and commit metadata across all refs. The author-approved cover is accepted only at its reviewed SHA-256 digest; changed or additional binary files require review.
