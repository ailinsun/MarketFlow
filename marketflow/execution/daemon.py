#!/usr/bin/env python3
"""The execution daemon: one tick of exit management and gated entry, run in a loop.

Each tick does, in order:

    1. read state        arm state, kill switch, global HALT, capital caps
    2. discover          the positions currently held (from config or the account)
    3. manage exits      for every held position, compare its payout probability with
                         the immediate-sale break-even and flag it for sale when the
                         position is worth less held than sold
    4. gated entry       consume queued entry and exit intents from configured signal
                         sources, each through the market-quality gate, the resolution
                         gate, the sizer and the capital caps
    5. record            append a structured ledger row and rewrite latest.json, the
                         heartbeat the money-path watchdog reads

Exits come first on purpose. An unattended position whose value has fallen below its
sale price is the failure a daemon exists to prevent; opening new positions is the
optional half and is gated much more heavily.

Capital protection is a set of fuses, none of which depends on a forecast being right:

  - dry run by default. Live execution needs both --live and an armed arm state, the
    single control file an operator writes; its mode (off / exit_only /
    entry_allowlisted / full) decides whether exits and entries may route live;
  - a kill file or a signal halts all execution immediately;
  - a global HALT stops buying while exits and redemptions continue;
  - per-trade and total caps gate entries, never exits;
  - an entry whose venue fee rate cannot be read is refused on the live path.

The daemon never builds or signs an order itself. Every live order is delegated to
marketflow.execution.orders, which re-checks arm state, caps, budget and kill against
the same files and is the last component able to refuse. Secrets are never printed
...
from marketflow.execution import orders as pmx  # noqa: E402  (the order module execution module; single-source arm-state + fuses)
...
DEFAULT_INTENT_QUEUE = runtime_path("execution", "polymarket_intents.jsonl")
# Single-source arm state shared with the order module. It is written only by an
# explicit operator action; this daemon and the order module only ever read it.
DEFAULT_ARM_STATE_FILE = pmx.DEFAULT_ARM_STATE_FILE


SCHEMA_VERSION = "polymarket-execution-daemon-v0.1"
INTENT_CONSUMED_SCHEMA_VERSION = "polymarket-intent-consumed-v0.1"
...
DEFAULT_MARKET_QUALITY = {
    "longshot_low": 0.15,
    "longshot_high": 0.85,
    "auto_band_low": 0.15,   # inner band collapsed onto the longshot guard: trade
...
# Post-only BUY pricing rule. "cheapest" = best_bid + tick, the cheapest price
# that can still rest; "queue_aware" = most fillable price that still clears the
# edge ceiling (see fill_model). The cheapest level is the default because live
# order prices are a money surface:
# switching costs one config key and one restart, but it is the operator's call, not a
# side effect of deploying this module.
DEFAULT_MAKER_PRICING = "cheapest"
SPEED_WINDOW_TAKER_REASON = "in_play_speed_window"
# Signal sources whose claim that a market resolves cleanly is accepted as a
# self-attestation: allowed, but every such entry raises an alert. Any other source
# needs an independent attestation or the daemon's own resolution verifier.
SELF_ATTESTED_RESOLUTION_SOURCES = {"discretionary_signal"}
...
BOUNDARIES = [
    ...
    "live_execution_delegated_to_the_order_module_fuses",
]
...
DAEMON_ARCHIVE_ROOT = runtime_path("archive", "execution-daemon-daily")
...
def append_jsonl_rotating(path: str, row: dict) -> None:
    ...
    Deliberately NOT used for the intent queues: _load_intent_consumed_keys
...
def _pending_intents_summary(queue_path: str = DEFAULT_INTENT_QUEUE, *, limit: int = 25) -> list:
    """Read-only summary of the intent queue minus consumed, mirrored into
    ...
    try:
        ...
        if not os.path.isfile(path):
            return []
        consumed = _load_intent_consumed_keys(_intent_consumed_path(path))
        out: list = []
...
def _intent_consumed_path(queue_path: str) -> str:
...
# The single outbound operator-alert path. It is a module-level name rather than a
# direct call so a self-test can replace it and prove the daemon sends nothing; an
# unconfigured sink drops the alert and returns False rather than raising.
from marketflow.monitor import notify as _notify  # noqa: E402

_operator_notify = _notify.send_alert
...
def _try_auto_clear_reserve_halt(*, min_age_sec: float = 1800.0) -> dict:
    ...
    passes the order module's authoritative per-order budget precheck, so this can never spend
...
def _load_intent_consumed_keys(consumed_path: str) -> set[str]:
    keys: set[str] = set()
...
def _fetch_market_rows(url: str, timeout: float = 5.0) -> list:
    ...
    """
    req = urllib.request.Request(url, headers={"User-Agent": "marketflow-execution-daemon"})
...
def maker_limit_price(candidate: dict, *, executable_price: Optional[float],
    ...
    Two rules live here. The cheapest-price rule bids `best_bid + tick`: the CHEAPEST
    ...
    distance from mid — where the cheapest-price rule deliberately picks the far end
    ...
    below best_ask, and every downstream gate (Kelly, caps, the order module) is untouched.
    """
    ...
    if queue_aware:
        ...
        # through to the cheapest-price rule and record why.
...
def fractional_kelly_buy_sizing(
    ...
    *,
    ...
    a conservative-quantile p_c = p - z*sigma_p (sized down by the candidate's probability
    uncertainty when present) and the verified per-share fee. The EV gate, caps,
    exchange-minimum floor and the order module fuses are unchanged. With no uncertainty info and a
    ...
    # Real balance is a THIRD ceiling next to per-trade and remaining cap.
...
class WinRateProvider:
    """the win-rate provider seam: returns the true held-side payout probability for a position.
...
class StubWinRateProvider(WinRateProvider):
    ...
    With no win-rate provider configured, the only honest win-rate source is an
    operator-supplied probability_override (interpreted as the held side's true
    payout probability).
    ...
    def get_win_probability(self, position_ctx: dict) -> WinRateResult:
        ...
        if p is None:
            return WinRateResult(
                ...
                notes="no win-rate provider configured and no probability_override supplied",
            )
        return WinRateResult(
            win_probability=p,
            ...
            notes="probability_override used as held-side payout probability (no win-rate provider configured)",
        )
...
class OrderIntent:
    ...
    price_floor_override_reason: Optional[str] = None  # carried to the order module's BUY price floor
...
class ExecutionAdapter:
    """the order module seam: turns an OrderIntent into a real CLOB place/cancel call.

    The real the order module adapter owns SDK signing, signature_type/funder wiring, and the
    AUTHORITATIVE capital fuse. The daemon only decides what to attempt and in
    which mode; the order module is the last line that can refuse.
    """
...
class DryRunStubExecutionAdapter(ExecutionAdapter):
    ...
    the order module is selected (--executor orders), no order is ever placed,
    ...
    def execute(self, intent: OrderIntent, *, effective_mode: str) -> ExecutionResult:
        if effective_mode == "live":
            return ExecutionResult(
                ...
                simulated=False,
                order_id=None,
                reason="live requested but no execution adapter is configured; no order placed (fail-safe)",
...
class OrderModuleExecutionAdapter(ExecutionAdapter):
    """the order module wiring: routes intents through the order module's canonical flow.

    Uses the order module's own plan_order -> (execute_order when will_execute_live) path so the order module's
    fuses (arm-state + kill + caps) stay authoritative. The daemon LiveGate only
    decides whether to REQUEST live; the order module decides whether to ACTUALLY execute. Live
    executed records are appended to the order module's own ledger so its cumulative deploy cap
    ...
    def execute(self, intent: OrderIntent, *, effective_mode: str) -> ExecutionResult:
        try:
            ...
            return ExecutionResult(False, False, effective_mode, False, None,
                                   f"order module unavailable: {exc}", asdict(intent))
        try:
            d = self._resolve(pmx)
            order_intent = self._s1_intent(pmx, intent, d)
            caps = pmx.owner_fuse_caps(max_total_deploy_usd=d["max_total_deploy_usd"], max_per_trade_usd=d["max_per_trade_usd"])
            book = None
            if d["estimate_fill"]:
                try:
                    book = pmx.fetch_book_readonly(order_intent.token_id)
                except Exception:
                    book = None
            plan = pmx.plan_order(
                order_intent,
            ...
            if plan.get("will_execute_live"):
                ...
                client = _with_hard_timeout(
                    ...
                    lambda: pmx.build_secure_client(
                        secrets, secret_dir=d["secret_dir"], side=order_intent.side
                    ),
                )
                try:
                ...
                if leaks:
                    ...
                    try:
                        pmx.engage_global_halt("secret leak guard tripped after live order submission")
                    ...
                    if self._alerter is not None:
                        self._alerter.alert("order_secret_leak_guard_tripped", payload)
        except Exception as exc:
            failure_intent = {"daemon": asdict(intent)}
            return ExecutionResult(False, False, effective_mode, False, None,
                                   f"order module execute failed: {exc}", failure_intent)
        ...
        result_intent = {"daemon": asdict(intent), "orders": record}
        return ExecutionResult(
            ...
            reason=f"order module {record.get('mode')}; would_place={plan.get('would_place')}; refusals={plan.get('fuses', {}).get('refusals')}",
...
class LiveGate:
    ...
    Single source of truth: it reads the same arm-state file the order module reads (via
    pmx.load_arm_state). Live requires --live AND an armed, operator-written,
    ...
    EXIT (exit_only). the order module's plan_order independently re-checks the same file per
    ...
    def resolve(self) -> dict:
        try:
            # Operator path: read the arm state with the operator ceiling, so caps the
            # operator set (up to the fat-finger ceiling) are honored as the effective
            # caps instead of flagged as tamper. A missing cap field falls back to
            # the default (the default fractions). External multi-tenant users run a SEPARATE
            # process with default FuseCaps.
...
class CapGuard:
    """Defense-in-depth capital fuse. the order module is authoritative; this pre-checks entries.
    ...
    def __init__(self, total_cap_usd: float, per_trade_cap_usd: float):
        ...
        #. None = unknown -> sizing keeps its cap-only behaviour and the
    ...
    def check_entry(self, notional_usd: Optional[float]) -> dict:
        ...
        if n > self.per_trade_cap_usd + 1e-9:
            return {
                "allowed": False,
                "reason": "entry exceeds per-trade cap (the order module is authoritative)",
                "advisory": True,
        ...
        if self.deployed_usd + n > self.total_cap_usd + 1e-9:
            return {
                "allowed": False,
                "reason": "entry would exceed total deployment cap (the order module is authoritative)",
                "advisory": True,
        ...
        return {
            "allowed": True,
            "reason": "within advisory caps; the order module makes the final call",
            "advisory": True,
    ...
    def reset_tick_window(self) -> None:
        ...
        entries (anti burst-buy). Cross-tick budget truth lives in the order module's account
...
def build_scale_in_intent(position_result: dict, *, add_shares: float, maker_price: float) -> OrderIntent:
    ...
    it never crosses/overpays; the order module's plan_order re-checks arm-state, caps, budget,
...
class ExitManager:
    ...
    def process_position(self, position_config: dict, *, effective_mode: str,
        ...
        if decision == "CLAIM_REDEEM_SIGNAL":
            ...
            record["execution"] = {
                "executed": False,
                "reason": "redeem/claim signal only; a live wallet claim requires an operator-supervised path",
            }
            self.alerter.alert(
...
class EntryManager:
    """Automated entry. Disabled unless a candidate source is configured and the
    arm-state mode permits BUY. Caps + live gate + edge all apply."""
    ...
    def __init__(
        ...
        *,
        ...
        # than the literal "queue_aware" keeps the cheapest rule — an unknown or
        ...
        # enforcement waits on the operator's read of the shadow evidence. Any unknown
        ...
        # Optional INDEPENDENT clean-resolution verifier. None => disabled (prior
    ...
    def _price_expression(self, candidate: dict, *, entry_price: Any,
                          model_probability: Any, record: dict,
                          effective_mode: str = "dry_run") -> dict:
    ...
    def _one_shot_consumed(record: dict) -> bool:
        """Consume one-shot keys only after the order module really accepted the attempt.

        Operational blocks (HALT, kill switch, SDK/proxy failures, bad/expired
        rows) must not burn an MarketFlow intent. the order module idempotency remains the
        ...
        if "duplicate" in reason and "idempotency" in reason:
            return True
        refusals = (
            ((execution.get("intent") or {}).get("orders") or {})
            .get("fuses", {})
            .get("refusals", [])
        )
        return any(
    ...
    def _sync_cap_guard_from_result(self, result: ExecutionResult) -> None:
        if not isinstance(result.intent, dict):
            return
        order_record = result.intent.get("orders")
        if not isinstance(order_record, dict):
            return
        fuses = order_record.get("fuses")
    ...
    def _mark_one_shot_consumed(self, candidate: dict, key: str, record: dict) -> None:
        ...
        if not consumed_path:
            return
        row = {
            "schema_version": INTENT_CONSUMED_SCHEMA_VERSION,
            "consumed_at": iso_now(),
            "idempotency_key": key,
        ...
        try:
            ...
            self.alerter.alert(
                "signal_intent_consumed_write_failed",
    ...
    def _process_candidate(self, candidate: dict, *, effective_mode: str) -> dict:
        ...
        if action == "SELL" or candidate_type == "signal_exit_intent":
            return self._process_signal_exit_intent(candidate, effective_mode=effective_mode)
        if candidate_type == "signal_intent":
            return self._process_signal_intent(candidate, effective_mode=effective_mode)
    ...
    def _process_signal_intent(self, candidate: dict, *, effective_mode: str) -> dict:
        ...
        if expires_at:
            try:
                ...
                if exp_dt <= datetime.now(timezone.utc):
                    rec["decision"] = "NO_ENTRY_INTENT_EXPIRED"
                    return rec
            except ValueError:
                rec["decision"] = "NO_ENTRY_BAD_INTENT_EXPIRY"
                return rec
        if close_time:
        ...
        if not bool(candidate.get("allow_live_entry")):
            rec["decision"] = "NO_ENTRY_INTENT_NOT_ENABLED"
            return rec
        if monitor.safe_str(candidate.get("action") or "BUY").upper() != "BUY":
            rec["decision"] = "NO_ENTRY_INTENT_ACTION_BLOCKED"
            rec["reason"] = "intent path accepts BUY only"
            return rec
        if monitor.safe_str(candidate.get("market_order_type") or "FOK").upper() != "FOK":
            rec["decision"] = "NO_ENTRY_INTENT_ORDER_TYPE_BLOCKED"
            rec["reason"] = "intent path accepts FOK only"
            return rec
        if max_price is None or max_price <= 0:
            rec["decision"] = "NO_ENTRY_BAD_INTENT"
            return rec
        ...
        # Re-price off the CURRENT book before any pricing decision. The
        ...
        # Objective filters bind before caps / arm-state / the order module. MarketFlow-directed
        ...
        if not cap["allowed"]:
            ...
            return rec
        intent = OrderIntent(
            intent_id=monitor.safe_str(candidate.get("idempotency_key") or f"intent_{now_ms()}_{hash_obj(candidate)[:10]}"),
            action="BUY",
            is_exit=False,
        ...
        rec["intent"] = asdict(intent)
        rec["decision"] = "INTENT_ENTRY_SIGNAL"
        if self.kill_switch.active():
            rec["execution"] = {"executed": False, "reason": "kill switch active; execution halted"}
            return rec
        result = self.executor.execute(intent, effective_mode=effective_mode)
        ...
        rec["execution"] = asdict(result)
        self.alerter.alert("signal_intent_entry_signal", {"intent": asdict(intent), "effective_mode": effective_mode})
        return rec

    def _process_signal_exit_intent(self, candidate: dict, *, effective_mode: str) -> dict:
        shares = to_float(candidate.get("shares"))
        held_shares = to_float(candidate.get("held_shares"))
        min_price = to_float(candidate.get("min_price"))
        rec: dict = {
            "stage": "exit",
            "candidate_type": "signal_exit_intent",
            "intent": None,
            "execution": None,
            "source": monitor.safe_str(candidate.get("source")),
            "min_price": min_price,
        ...
        if expires_at:
            try:
                exp_dt = datetime.fromisoformat(expires_at.replace("Z", "+00:00"))
                if exp_dt <= datetime.now(timezone.utc):
                    rec["decision"] = "NO_EXIT_INTENT_EXPIRED"
                    return rec
            except ValueError:
                rec["decision"] = "NO_EXIT_BAD_INTENT_EXPIRY"
                return rec
        if not bool(candidate.get("allow_live_exit")):
            rec["decision"] = "NO_EXIT_INTENT_NOT_ENABLED"
            return rec
        order_type = monitor.safe_str(candidate.get("market_order_type") or "FAK").upper()
        if order_type not in ("FAK", "FOK"):
            rec["decision"] = "NO_EXIT_INTENT_ORDER_TYPE_BLOCKED"
            rec["reason"] = "intent exit path accepts FAK or FOK"
            return rec
        if shares is None or shares <= 0 or held_shares is None or held_shares <= 0:
            rec["decision"] = "NO_EXIT_BAD_INTENT"
        ...
        if min_price is None or min_price <= 0 or min_price > 1:
            ...
            return rec
        intent = OrderIntent(
            intent_id=monitor.safe_str(candidate.get("idempotency_key") or f"intent_exit_{now_ms()}_{hash_obj(candidate)[:10]}"),
            action="SELL",
            is_exit=True,
            market_id=monitor.safe_str(candidate.get("market_id")),
            market_slug=monitor.safe_str(candidate.get("market_slug")),
        ...
        rec["intent"] = asdict(intent)
        rec["decision"] = "INTENT_EXIT_SIGNAL"
        if self.kill_switch.active():
            rec["execution"] = {"executed": False, "reason": "kill switch active; execution halted"}
            return rec
        result = self.executor.execute(intent, effective_mode=effective_mode)
        rec["execution"] = asdict(result)
        self.alerter.alert("signal_intent_exit_signal", {"intent": asdict(intent), "effective_mode": effective_mode})
        return rec
...
def default_config() -> dict:
    return {
        ...
        "enable_model_intents": True,
        "model_intent_queue_files": [DEFAULT_INTENT_QUEUE],
        "model_intent_queue_tail": 25,
...
def cap_consistency_report(config_path: Optional[str] = None) -> dict[str, Any]:
    ...
    order_default = OrderModuleExecutionAdapter({})._resolve(pmx)
    config_cap: dict[str, Any] = {}
    config_orders: dict[str, Any] = {}
    if config_path and os.path.exists(config_path):
        ...
        if isinstance(cfg, dict):
            ...
            config_orders = orders_cfg if isinstance(orders_cfg, dict) else {}
    values = {
        ...
        "order_default_total_cap_usd": order_default.get("max_total_deploy_usd"),
        "order_default_per_trade_cap_usd": order_default.get("max_per_trade_usd"),
        ...
        "config_orders_total_cap_usd": config_orders.get("max_total_deploy_usd", source_total),
        "config_orders_per_trade_cap_usd": config_orders.get("max_per_trade_usd", source_per_trade),
    }
    ok = (
        ...
        and abs(float(values["order_default_total_cap_usd"]) - source_total) < 1e-9
        and abs(float(values["order_default_per_trade_cap_usd"]) - source_per_trade) < 1e-9
        ...
        and abs(float(values["config_orders_total_cap_usd"]) - source_total) < 1e-9
        and abs(float(values["config_orders_per_trade_cap_usd"]) - source_per_trade) < 1e-9
    )
...
def read_available_collateral_usd(
    *,
    ...
    Why the daemon needs this: caps are a POLICY ceiling ("how much are
...
def _load_intent_queue(
    ...
    *,
    ...
    if not os.path.isfile(path):
        return []
    consumed_path = _intent_consumed_path(path)
    try:
        consumed_keys = _load_intent_consumed_keys(consumed_path)
    except OSError as exc:
        record = _entry_source_error_record(source="signal_intent_consumed", path=consumed_path, exc=exc)
        if alerter is not None:
            alerter.alert("signal_intent_consumed_read_failed", record)
        return [record]
    rows: list[dict] = []
    try:
        with open(path, encoding="utf-8") as f:
        ...
        record = _entry_source_error_record(source="signal_intent_queue", path=path, exc=exc)
        if alerter is not None:
            alerter.alert("signal_intent_queue_read_failed", record)
        return [record]
    for line in lines[-max(1, min(int(limit or 25), 200)):]:
        ...
        try:
            ...
            payload = exception_payload(exc, source="signal_intent_queue", path=path)
            if alerter is not None:
                alerter.alert("signal_intent_queue_bad_json", payload)
            rows.append({
                "stage": "entry",
                "candidate_type": "entry_source_error",
                "allow_live_entry": False,
                "decision": "NO_ENTRY_BAD_INTENT_JSON",
        ...
        if not isinstance(row, dict):
            continue
        if row.get("schema_version") != "polymarket-intent-v0.1":
            continue
        ...
        candidate["candidate_type"] = "signal_exit_intent" if action == "SELL" else "signal_intent"
        if action == "SELL":
...
def make_entry_candidate_fn(config: dict, *, alerter: Optional[Alerter] = None) -> Optional[Callable[[], list]]:
    ...
    discretionary_paths = list(config.get("discretionary_intent_queue_files") or [])
    allow_discretionary = bool(config.get("enable_discretionary_intents"))
    discretionary_limit = int(config.get("discretionary_intent_queue_tail", 25))
    model_paths = list(config.get("model_intent_queue_files") or [])
    allow_model = bool(config.get("enable_model_intents"))
    model_limit = int(config.get("model_intent_queue_tail", discretionary_limit))
    if not candidates and not discretionary_paths and not model_paths:
        return None

    def _load() -> list:
        out = [dict(c) for c in candidates]
        for path in model_paths:
            out.extend(_load_intent_queue(
                str(path),
                allow_live_entry=allow_model,
                allow_live_exit=allow_model,
                limit=model_limit,
                alerter=alerter,
                source_filter={"model_signal"},
            ))
        for path in discretionary_paths:
            out.extend(_load_intent_queue(
                str(path),
                allow_live_entry=allow_discretionary,
                allow_live_exit=allow_discretionary,
                limit=discretionary_limit,
                alerter=alerter,
                source_exclude={"model_signal"},
            ))
        return out
...
def make_executor(name: str, config: dict, *, alerter: Optional[Alerter] = None) -> ExecutionAdapter:
    ...
    if key in ("orders", "order_module"):
        return OrderModuleExecutionAdapter(config, alerter=alerter)
...
class AutotradeDaemon:
    def __init__(self, args: argparse.Namespace):
        ...
        self.win_provider: WinRateProvider = make_win_provider(win_name, self.config.get("win_rate", {}))
        self.executor: ExecutionAdapter = make_executor(exec_name, self.config.get("orders", {}), alerter=self.alerter)
    ...
    def tick(self) -> dict:
        ...
        # Sync the CapGuard ceiling from the operator arm-state's effective caps so
        # caps set in the arm-state take effect this tick. None (no cap field in
        # the arm-state) leaves the config-derived caps untouched. Only raises or
        # lowers this single-account ceiling (delegated mandates run in a separate
        ...
        if kill.get("global_halt_active"):
            ...
            if auto_clear.get("cleared"):
                ...
                if not getattr(self, "_halt_alert_sent", False):
                    ...
                    self._halt_alert_sent = _operator_notify(
        ...
        # Real spendable balance -> sizing's third ceiling. Only read when a
        ...
        if positions_state is POSITION_DISCOVERY_FAILED:
            record["entries_skipped"] = {
                "reason": "position discovery failed; holding this tick before any entry",
                "positions_state": "unknown",
            }
        ...
        for position_config in positions_state:
            ...
            # the exit bankroll unlocks target-driven scale-in (favourable add) inside
        ...
        # them, avoiding dry-run one-shot burn before the operator arms exits.
    ...
    def run(self) -> int:
        ...
        try:
            ...
            while True:
                ...
                # File-based kill (emergency stop) must NOT exit the process: the
                # per-tick kill_switch.active() checks already block every
                # execution, so the daemon stays resident in dry_run. Exiting on a
                # file kill would fight a supervisor's restart policy into a respawn
...
def human_summary(record: dict) -> str:
    ...
    lines = [
        ...
        f"- executor: `{record.get('executor')}` (dry_run_stub = places nothing / order module = through the execution fuses)",
    ...
    if not positions and (record.get("position_discovery") or {}).get("status") == "failed":
        lines.append("- position state unknown; entry was skipped this tick")
    elif not positions:
...
def _selftest_body(fee_fetch_calls: list, book_fetch_calls: list) -> dict:
    ...
    # Operator daemon path: the order-module adapter builds caps via owner_fuse_caps,
    # so the operator's own daemon CAN raise its caps above the default -- up to the
    # operator fat-finger ceiling; a stray extra zero past the ceiling still clamps down.
    hi = OrderModuleExecutionAdapter({"max_total_deploy_usd": 500.0, "max_per_trade_usd": 50.0})
    ...
    fat = OrderModuleExecutionAdapter({"max_total_deploy_usd": 1.0e9, "max_per_trade_usd": 1.0e9})
    ...
    # real balance is a third sizing ceiling beside per-trade / remaining cap.
    ...
    sent: list[str] = []
    real_notify = globals()["_operator_notify"]
    globals()["_operator_notify"] = lambda text: (sent.append(text), True)[1]
    try:
        ...
        globals()["_operator_notify"] = lambda text: False
        for _ in range(AutotradeDaemon.EXIT_FAIL_ALERT_STREAK):
            fail_daemon._note_exit_failure("mid:tid", cfg, RuntimeError("ESPN down"))
        ...
        globals()["_operator_notify"] = real_notify
    ...
    order_adapter = OrderModuleExecutionAdapter({
    ...
    order_dry = order_adapter.execute(order_intent, effective_mode="dry_run")
    _assert(checks, "order_adapter_dry_run_no_execution", order_dry.executed is False and order_dry.simulated is True)
    _assert(checks, "order_adapter_routes_through_the_order_plan", "DRY_RUN_PLAN" in (order_dry.reason or ""))
    order_live_no_ack = order_adapter.execute(order_intent, effective_mode="live")
    _assert(checks, "order_adapter_live_without_ack_no_execution", order_live_no_ack.executed is False)
    _assert(checks, "order_adapter_no_ledger_write_in_dry_run", not os.path.exists(s1_tmp_ledger))
    ...
    _assert(checks, "factory_real_adapters", make_executor("orders", {}).name == "order_module")
    ...
    # market-quality gate on the intent entry path
    def _entry_mgr(cands, market_quality=None, alerter=None, resolution_verifier=None,
        ...
        return EntryManager(
            win_provider=StubWinRateProvider(),
            executor=DryRunStubExecutionAdapter(),
            cap_guard=CapGuard(DEFAULT_TOTAL_CAP_USD, per_trade_cap),
            kill_switch=KillSwitch(kill_file="/no/such/kill"),
    ...
    _base_intent = {
        "candidate_type": "signal_intent", "allow_live_entry": True, "one_shot": True, "source": "discretionary_signal",
    ...
    _assert(checks, "signal_intent_clean_passes_gate", clean_rec["decision"] == "INTENT_ENTRY_SIGNAL")
    ...
    _assert(checks, "signal_intent_record_carries_price_expression",
    ...
    # live re-pricing: an intent's ask_price is stale by the time it is
    # processed. Unreachable book (selftest token) must fall back to it, not crash.
    _assert(checks, "signal_intent_stale_price_fallback_when_book_unreachable",
    ...
    _assert(checks, "signal_intent_reprices_off_live_book",
    ...
    _assert(checks, "signal_intent_refuses_when_price_ran_past_max",
    ...
    # The cheap rule takes the lowest level (0.41, farthest from the mid = least fillable);
    ...
    try:
        ...
        _cheapest = _entry_mgr([_wide_cand]).tick(effective_mode="dry_run")[0]
        ...
        # must hand back to the cheapest-price rule rather than refuse an authorised entry.
        ...
        globals()["_live_book_quote"] = _saved_quote
    _assert(checks, "maker_pricing_defaults_to_the_cheapest_level",
            _cheapest.get("maker_price", {}).get("basis") == "best_bid_plus_tick"
            and abs(float(_cheapest["maker_price"]["price"]) - 0.41) < 1e-9)
    # Ceiling is the LIVE ask (0.50) — the price the edge test just cleared — so the
    # most fillable maker level is one tick under it, eight ticks above the cheap rule's 0.41.
    ...
    _assert(checks, "maker_pricing_unknown_value_stays_cheapest",
    ...
    _assert(checks, "maker_pricing_queue_aware_falls_back_when_book_unusable",
            ...
            and _no_depth.get("decision") == "INTENT_ENTRY_SIGNAL")
    clean_intent = clean_rec.get("intent") or {}
    requested_sizing = clean_rec.get("requested_sizing") or {}
    _assert(checks, "signal_intent_uses_kelly_maker_not_requested_size",
    ...
    _assert(checks, "signal_intent_longshot_rejected",
    ...
    _assert(checks, "signal_intent_no_model_prob_informational",
    ...
    _assert(checks, "signal_intent_no_model_prob_strict_config_rejected",
    ...
    _assert(checks, "signal_intent_low_private_edge_informational",
    ...
    _assert(checks, "signal_intent_low_private_edge_strict_config_rejected",
    ...
    _assert(checks, "one_shot_expired_row_does_not_burn_later_same_key",
            same_key_records[0]["decision"] == "NO_ENTRY_INTENT_EXPIRED"
            and same_key_records[1]["decision"] == "INTENT_ENTRY_SIGNAL")
    ...
    retry_mgr = EntryManager(
        win_provider=StubWinRateProvider(),
        executor=retry_exec,
        cap_guard=CapGuard(DEFAULT_TOTAL_CAP_USD, 1.0),
        kill_switch=KillSwitch(kill_file="/no/such/kill"),
        alerter=Alerter(enabled=False, alert_file="/tmp/none"),
        edge_buffer=0.05, fee_bps=0.0,
    ...
    budget_cap = CapGuard(DEFAULT_TOTAL_CAP_USD, 1.0)
    budget_mgr = EntryManager(
        win_provider=StubWinRateProvider(),
        executor=_OrderBudgetEchoExecutor(),
    ...
    _assert(checks, "entry_manager_syncs_cap_from_order_budget",
            budget_rec["decision"] == "INTENT_ENTRY_SIGNAL"
            and abs(budget_cap.deployed_usd - 12.0) < 1e-9
            and "selftest_budget_sync" in budget_mgr._attempted_one_shot_ids)

    order_entry_ledger = os.path.join(OUT_DIR, "selftest_order_entry_ledger_absent.jsonl")
    if os.path.exists(order_entry_ledger):
        os.remove(order_entry_ledger)
    order_entry_mgr = EntryManager(
        win_provider=StubWinRateProvider(),
        executor=OrderModuleExecutionAdapter({
            "kill_file": "/no/such/kill",
            "arm_state_file": "/no/such/arm.json",
            "ledger_path": order_entry_ledger,
            "estimate_fill": False,
        }),
        cap_guard=CapGuard(DEFAULT_TOTAL_CAP_USD, 1.0),
        kill_switch=KillSwitch(kill_file="/no/such/kill"),
    ...
    order_entry_rec = order_entry_mgr.tick(effective_mode="dry_run")[0]
    order_entry_plan = (order_entry_rec.get("execution") or {}).get("intent", {}).get("orders", {})
    _assert(checks, "signal_intent_private_signal_routes_to_order_module_dry_plan",
            order_entry_rec["decision"] == "INTENT_ENTRY_SIGNAL"
            and (order_entry_rec.get("execution") or {}).get("accepted") is True
            and (order_entry_rec.get("execution") or {}).get("executed") is False
            and order_entry_plan.get("mode") == "DRY_RUN_PLAN"
            and order_entry_plan.get("would_place") is True)
    _assert(checks, "order_entry_dry_plan_no_ledger_write", not os.path.exists(order_entry_ledger))
    ...
    try:
        ...
        leak_res = OrderModuleExecutionAdapter(
            {
                "kill_file": "/no/such/kill",
    ...
    leak_order = leak_res.intent.get("orders") if isinstance(leak_res.intent, dict) else {}
    ...
    _assert(checks, "leak_guard_triggers_halt_and_alert",
            ...
            and "order_secret_leak_guard_tripped" in open(leak_alert, encoding="utf-8").read()
            and ((leak_order.get("secret_leak_guard") or {}).get("halt_engaged") is True))
    for path in (leak_ledger, leak_arm, leak_alert, tmp_global_halt):
        try:
            os.remove(path)
        except FileNotFoundError:
    ...
    replay_queue = os.path.join(OUT_DIR, "selftest_intents_replay.jsonl")
    replay_consumed = _intent_consumed_path(replay_queue)
    ...
    replay_row = dict(
        _base_intent,
        schema_version="polymarket-intent-v0.1",
        created_at=iso_now(),
        expires_at="2999-01-01T00:00:00Z",
        source="discretionary_signal",
        idempotency_key=replay_key,
    )
    append_jsonl(replay_queue, replay_row)
    replay_loader = make_entry_candidate_fn(
        {
            "discretionary_intent_queue_files": [replay_queue],
            "discretionary_intent_queue_tail": 25,
            "enable_discretionary_intents": True,
        },
        alerter=Alerter(enabled=False, alert_file="/tmp/none"),
    )
    ...
    _assert(checks, "signal_intent_consumed_key_persisted",
            len(replay_first) == 1
            and replay_first[0].get("decision") == "INTENT_ENTRY_SIGNAL"
            and os.path.exists(replay_consumed)
            and replay_key in open(replay_consumed, encoding="utf-8").read())
    _assert(checks, "signal_intent_consumed_key_not_replayed_after_restart", replay_second == [])
    for path in (replay_queue, replay_consumed):
        try:
            os.remove(path)
        except FileNotFoundError:
    ...
    model_queue = os.path.join(OUT_DIR, "selftest_model_intents.jsonl")
    model_consumed = _intent_consumed_path(model_queue)
    for path in (model_queue, model_consumed):
        try:
            os.remove(path)
        except FileNotFoundError:
            pass
    model_buy_key = "selftest_model_buy"
    discretionary_buy_key = "selftest_chat_buy"
    append_jsonl(model_queue, dict(
        _base_intent,
        schema_version="polymarket-intent-v0.1",
        created_at=iso_now(),
        expires_at="2999-01-01T00:00:00Z",
        source="model_signal",
        idempotency_key=model_buy_key,
        model_version="selftest",
        feature_hash="sha256:selftestmodel",
    ))
    append_jsonl(model_queue, dict(
        _base_intent,
        schema_version="polymarket-intent-v0.1",
        ...
        expires_at="2999-01-01T00:00:00Z",
        source="discretionary_signal",
        idempotency_key=discretionary_buy_key,
    ))
    model_loader = make_entry_candidate_fn(
        {
            "model_intent_queue_files": [model_queue],
            "model_intent_queue_tail": 25,
            "enable_model_intents": True,
            "discretionary_intent_queue_files": [model_queue],
            "discretionary_intent_queue_tail": 25,
            "enable_discretionary_intents": True,
        },
        alerter=Alerter(enabled=False, alert_file="/tmp/none"),
    )
    model_loaded = model_loader() if model_loader else []
    _assert(checks, "model_queue_precedes_discretionary_queue_without_duplicate",
            len(model_loaded) == 2
            and model_loaded[0].get("source") == "model_signal"
            and model_loaded[0].get("idempotency_key") == model_buy_key
            and model_loaded[1].get("source") == "discretionary_signal"
            and model_loaded[1].get("idempotency_key") == discretionary_buy_key)

    model_sell_key = "selftest_model_sell"
    append_jsonl(model_queue, {
        "schema_version": "polymarket-intent-v0.1",
        "created_at": iso_now(),
        "expires_at": "2999-01-01T00:00:00Z",
        "source": "model_signal",
        ...
        "idempotency_key": model_sell_key,
    })
    model_sell_loader = make_entry_candidate_fn(
        {
            "model_intent_queue_files": [model_queue],
            "model_intent_queue_tail": 25,
            "enable_model_intents": True,
            "discretionary_intent_queue_files": [model_queue],
            "discretionary_intent_queue_tail": 25,
            "enable_discretionary_intents": False,
        },
        alerter=Alerter(enabled=False, alert_file="/tmp/none"),
    )
    model_sell_loaded = model_sell_loader() if model_sell_loader else []
    sell_rows = [r for r in model_sell_loaded if r.get("idempotency_key") == model_sell_key]
    _assert(checks, "model_sell_not_blocked_by_agent_entry_toggle",
            len(sell_rows) == 1
            and sell_rows[0].get("candidate_type") == "signal_exit_intent"
    ...
    _assert(checks, "signal_intent_missing_resolution_rejected",
    ...
    _assert(checks, "signal_intent_dirty_resolution_rejected",
    ...
    _assert(checks, "signal_intent_dirty_resolution_config_false_still_rejected",
    ...
    _assert(checks, "signal_intent_self_attested_resolution_passes_with_alert",
            self_attested_rec["decision"] == "INTENT_ENTRY_SIGNAL"
    ...
    # A live taker entry whose venue fee rate cannot be read is refused end to end.
    # The self-test's fee fetch returns nothing, so every rate here is unknown: the
    # same intent must be priced in a dry run and refused in live mode.
    _taker = dict(_base_intent, idempotency_key="selftest_unknown_fee_taker", speed_window=True,
                  taker_allowed_reason=SPEED_WINDOW_TAKER_REASON)
    _live_rec = _entry_mgr([_taker]).tick(effective_mode="live")[0]
    _assert(checks, "live_taker_entry_with_unknown_fee_is_refused",
            _live_rec["decision"] == "NO_ENTRY_PRICE_EXPRESSION_REJECT"
            and "FEE_RATE_UNKNOWN" in _live_rec.get("reject_reason_codes", []))
    _dry_rec = _entry_mgr([dict(_taker, idempotency_key="selftest_unknown_fee_taker_dry")]).tick(
        effective_mode="dry_run")[0]
    _assert(checks, "dry_run_taker_entry_with_unknown_fee_is_priced_not_refused",
            "FEE_RATE_UNKNOWN" not in (_dry_rec.get("reject_reason_codes") or [])
            and (_dry_rec.get("price_expression") or {}).get("fee_rate_source") == "fallback")

    # --- independent resolution verifier integration (model-signal path) -------
    # A model-signal intent ships resolution_confirmed_clean=False (never
    ...
    model_intent = dict(
        _base_intent, source="model_signal", resolution_confirmed_clean=False,
    ...
    verifier_clean_rec = _entry_mgr([model_intent], resolution_verifier=_clean_verifier).tick(effective_mode="dry_run")[0]
    _assert(checks, "verifier_independent_clean_passes_gate",
            verifier_clean_rec["decision"] == "INTENT_ENTRY_SIGNAL"
    ...
    verifier_dirty_rec = _entry_mgr(
        [dict(model_intent, idempotency_key="selftest_verifier_dirty")],
    ...
    verifier_err_rec = _entry_mgr(
        [dict(model_intent, idempotency_key="selftest_verifier_err")],
    ...
    # Verifier never overrides an already-disabled (None) verifier: the prior path.
    verifier_off_rec = _entry_mgr([dict(model_intent, idempotency_key="selftest_verifier_off")]).tick(effective_mode="dry_run")[0]
    ...
    # Exercise both branches: no config file, and a config file that restates the
    # module defaults (the branch that reads it must actually run).
    cap_report = cap_consistency_report(None)
    _assert(checks, "cap_sources_consistent", cap_report["ok"])
    cap_cfg_path = os.path.join(OUT_DIR, "selftest_cap_consistency.json")
    write_json(cap_cfg_path, {
        "capital": {"total_cap_usd": pmx.DEFAULT_MAX_TOTAL_DEPLOY_USD,
                    "per_trade_cap_usd": pmx.DEFAULT_MAX_PER_TRADE_USD},
        "orders": {"max_total_deploy_usd": pmx.DEFAULT_MAX_TOTAL_DEPLOY_USD,
                   "max_per_trade_usd": pmx.DEFAULT_MAX_PER_TRADE_USD},
    })
    _assert(checks, "cap_config_file_consistent", cap_consistency_report(cap_cfg_path)["ok"])
    write_json(cap_cfg_path, {"orders": {"max_total_deploy_usd": pmx.DEFAULT_MAX_TOTAL_DEPLOY_USD * 2}})
    _assert(checks, "cap_config_file_drift_detected", not cap_consistency_report(cap_cfg_path)["ok"])
    os.remove(cap_cfg_path)
    ...
    report = {
        "schema_version": "polymarket-execution-daemon-selftest-v0.1",
        "generated_at": iso_now(),
        "PASS": ok,
        "checks": checks,
...
def _example_config() -> dict:
    return {
        ...
        "market_quality": dict(DEFAULT_MARKET_QUALITY),
        # Discretionary intents: queued rows from any allow-listed source other than
        # model_signal. Default OFF (safe): discretionary entry stays a deliberate
        # opt-in. Every queued intent still passes objective market filters
        # (longshot/band), private edge policy, caps, and arm-state before any
        # sign/POST.
        "enable_discretionary_intents": False,
        "discretionary_intent_queue_files": [DEFAULT_INTENT_QUEUE],
        "discretionary_intent_queue_tail": 25,
        # Intents from a deterministic model signal source share the durable queue.
        # On by default because only source=model_signal rows are read here: an
        # empty queue changes nothing. The order-module fuses and arm state stay
        # final either way.
        "enable_model_intents": True,
        "model_intent_queue_files": [DEFAULT_INTENT_QUEUE],
        "model_intent_queue_tail": 25,
        ...
        # (sports/esports) + standard-UMA + undisputed markets, so model-signal
        # intents, which never self-attest, can still clear the
        # resolution fuse. Fail-closed; it never arms or weakens caps, kill or arm
        # state. Set enabled=false for attestation-only behaviour.
        "resolution_verifier": {"enabled": True, "fetch_timeout_sec": rverify.DEFAULT_FETCH_TIMEOUT},
        "alert": {"enabled": False},
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Exit-first execution daemon (dry_run by default).")
    ...
    p.add_argument("--arm-state-file", default=DEFAULT_ARM_STATE_FILE, help="Unified arm-state JSON, shared with the order module and written only by an operator action.")
    ...
    p.add_argument("--executor", choices=["stub", "orders"], help="Execution adapter; overrides config.executor (default stub).")
